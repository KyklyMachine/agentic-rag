...
async def search_documents(vector_db: VectorDBDep, search_params: SearchParamDep, index_name: str, query: str, embedder: EmbedderDep):
...
async def add_documents(vector_db: VectorDBDep, index_name: str, document: Document, embedder: EmbedderDep) -> None:
    try:
...
async def delete_documents(vector_db: VectorDBDep, index_name: str, documents_ids: list[UUID]):
